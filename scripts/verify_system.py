...
def check_nlp_endpoint(base: str, token: str) -> bool:
    status, data = _request("POST", f"{base}/api/consultation/process-text",
        {"text": "Patient reports severe chest pain and difficulty breathing since morning"},
        token=token,
    )
    if status == 200:
        ...
        return True
    _fail(f"NLP endpoint failed (HTTP {status}): {data.get('detail', data)}")
    return False


def check_appointment_booking(base: str, token: str) -> bool:
...
def run_all_checks(base: str) -> int:
    ...
    # ── Section 4: Dashboard & AI ────────────────────────────────────────
    print(f"{BOLD}[4/5] Dashboard & AI Pipeline{RESET}")
    if doc_token:
        for fn in (check_dashboard_overview, check_doctor_queue, check_nlp_endpoint):
            if not fn(base, doc_token):
                failures += 1
    else:
    ...
    # ── Section 5: End-to-End booking ───────────────────────────────────
    print(f"{BOLD}[5/5] End-to-End Appointment Booking{RESET}")
    if pat_token:
        if not check_appointment_booking(base, pat_token):
            failures += 1
    else:
        _warn("Skipping booking check — patient token unavailable")
        failures += 1
    print()
